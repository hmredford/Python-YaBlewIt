...
class Game:
    ...
    def addcardtoclaim(self):
        if self.deck[-1].name != "fire in the hole":
    ...
    def askplayers(self):
        ...
        while len(self.players_out) < (self.num_players - 1):
            if (self.current_player != self.prospector) and (self.current_player not in self.players_out):
    ...
    def begin(self):
        while len(self.deck) > 0:
            self.newround()
            self.addtoclaim()
            print(f"\n{self.players[self.prospector].name} is the prospector!")
            self.printclaim()
